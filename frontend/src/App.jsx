...
export default function App() {
  ...
  const [sendBtn, setSend] = useState(false);
  ...
  const onZipAndSend = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "your-hosted-backend-url/upload",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleCleanup = async () => {
    try {
      ...
      const response = await fetch(
        "your-hosted-backend-url/cleanup",
        {
          method: "POST",
        }
      );
  ...
  /*const handleDownload = async () => {
    try {
      const response = await fetch("http://0.0.0.0:8080/download-zip", {
        method: "GET",
      });
      ...
      <div className=" flex justify-center gap-16 mt-6 px-10">
        <div className="border-2 rounded-xl p-6 bg-gray-100 lg:w-1/4 md:w-1/2 sm:w-full sm:m-6">
          ...
          <div>
            <div className="m-2">
              <input
                className="shadow appearance-none border rounded w-3/4 py-2 px-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                id="username"
                type="number"
                min="25"
                max="75"
                placeholder="Compression % (Int)"
                onChange={(e) => {
                  setCompressionPercentage(e.target.value);
                }}
              />
            </div>
            <div>
              {sendBtn ? (
                ...
                <button className="text-md font-bold m-2  bg-transparent hover:bg-blue-500 text-blue-700 hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded">
